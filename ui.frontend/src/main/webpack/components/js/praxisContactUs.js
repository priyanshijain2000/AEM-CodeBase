...
$(function() {
    ...
    function updateCards(list) {
        ...
        for (var i = 0; i <= list.length - 1; i++) {
            ...
            if(list[i]['Subtitle'])
                row+='<tr><td colspan=2 class="table-note"><h4>'+  list[i]['Subtitle']+'</h4></td></tr>';
            if(list[i]['Note']){
                if(list[i]['Note URL']) {
            ...
            $(row).appendTo(".praxis-contactus-filter__table__content");
        }
    }
    ...
    function updateDropDown(data) {
        ...
        // $('.praxis-contactus-filter .list-autocomplete').empty();
        // Country.forEach(function(value) {
        //     $('.praxis-contactus-filter .list-autocomplete').append(' <button type="button" class="dropdown-item" value = "' + value['Title'] + '"><label>' + value['Title'] + '</label></button>')
        // });
        Country.forEach(function(value) {
            $('.praxis-contactus-dropdown').append('<option>' + value['Title'] + '</option>')
        });
        $(".praxis-contactus-dropdown").select2({
			placeholder: $(".praxis-contactus-dropdown").data("placeholder"),
			//maximumSelectionLength: 2,
			selectionCssClass: "singleselect-withsearch",
            dropdownCssClass: "singleselect-withsearch",
		});

        // if ($(".praxis-contactus-filter")[0]) {

        //     $('.praxis-contactus-filter .dropdown-item').on('click', function() {
        //         var category = $(this).attr("searchData");
        //         console.log(category);
        //         $('.praxis-contactus-filter .jAuto').val($(this).children('label').html());
        //         var Country = $(this).children('label').html();

        //         var onloadCountrydata = filterData(data, Country, 'Title');

        //         $('.praxis-contactus-filter__table__content').empty();
        //         updateCards(onloadCountrydata);
        //     });
        // }
        if ($(".praxis-contactus-dropdown")[0]) {

            $('.praxis-contactus-dropdown').on('change', function() {
                var Country  = $(".praxis-contactus-dropdown option:selected").text();
    ...
    if ($(".praxis-contactus-dropdown")[0]) {
        var dataPath = $("#praxis-contactus").data('praxis-contactus-path');
        $.getJSON(dataPath, function(data) {
            var filterdropDown = data["contact_us_list_praxis"];
    ...
    // $('.praxis-contactus-filter__filter').on('focus', '.dropdown-item', function() {
    //     var element = $(this);
    //     var categoryElement = element[0];
    //     var category = $(categoryElement).attr("searchData");
    //     console.log(category);
    //     $('.praxis-contactus-filter .jAuto').val($(categoryElement).children('label').html());
    ...
    // $('.praxis-contactus-filter__filter .jAuto').keypress(function(event){
    //     var keycode = (event.keyCode ? event.keyCode : event.which);
    //     if(keycode == '13'){
    //      $('.praxis-contactus-filter__filter .dropdown-menu').addClass('show');
    //     $('.praxis-contactus-filter__filter .dropdown').addClass('active-dropdown');
    
    //     }
    //     event.stopPropagation();
    // });
    // $('.praxis-contactus-filter__filter .dropdown-menu').keypress(function(event){
    //     var keycode = (event.keyCode ? event.keyCode : event.which);
    //     if(keycode == '13'){
    //      $('.praxis-contactus-filter__filter .dropdown-menu').removeClass('show');
    //     $('.praxis-contactus-filter__filter .dropdown').removeClass('active-dropdown');
    
    //     }
    //     event.stopPropagation();
    // });
    // $(".praxis-contactus-filter__filter .jAuto").on("keyup", function(e) {
