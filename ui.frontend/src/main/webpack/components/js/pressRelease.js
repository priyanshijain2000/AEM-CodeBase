...
/*
$(function () {
    function updateCards(loadmoreCount, maxvalidcard, cards, initiate) {
        ...
        while (i < maxvalidcard) {
            if (i > loadmoreCount - 1) {
                ...
                var cardDate = new Date(cards[i].date).toLocaleDateString('en-us', { month: "long", year: "numeric", day: "numeric" });
                var card = ' <div class="card press-release-cards-container info-card-translucent"><img src="' + cards[i].image + '" class="card-img-top" alt=""><div class="card-body"><p class="body-text press-release-date">' + cardDate + '</p><h5>' + cards[i].title + '</h5><div class="card-heading-seperator"><span class="card-heading-seperator-stroke-big"></span><span class="card-heading-seperator-stroke-small"></span></div><p class="press-release-cards-container-content body-text">' +
                    cards[i].description + '</p> </div><div class="card-footer"> <div class="article-banner-button-band m-0"><a class="cta-btn-primary" href="' +
                    cards[i].path + '" aria-label="Read More About '+cards[i].title+'">learn more</a>        </div>    </div></div>';
                $(card).appendTo(".press-release-cards");
                i++;
                continue;
            } else {
    ...
    function initiateLoadmore(totalloadedData,loadmoreCount, maxvalidcard, cards,initiate) {
        ...
        if (screen.width <= 768) {
           ...
           var maxvalidcard = totalloadedData + 6;                        
        }
        totalloadedData = +updateCards(loadmoreCount, maxvalidcard, cards, initiate);
        // Start Rellax
        // var rellax = new Rellax('.rellax');
    ...
    if ($(".press-release-cards")[0]) {
        ...
        $.getJSON(dataPath, function (data) {
            ...
            $('.press-release-load-more .load-more-button').on('click', function() {
            ...
            $('.press-release-dropdown ').on('change', function () {
                ...
                category = $(".press-release-dropdown option:selected").val();
                dataPath = $("#press-release-group").data('press-release-path');
                dataPath = "/src/main/webpack" + dataPath + category; // dataPath="/bin/ets/fetchNewsPages.json?path="+dataPath+"&category="+category;
                $.getJSON(dataPath, function (data) {
