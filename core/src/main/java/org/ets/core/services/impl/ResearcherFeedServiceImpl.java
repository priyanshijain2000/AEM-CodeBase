...
public class ResearcherFeedServiceImpl implements ResearcherFeedService {
    ...
    public void createJsonFeed(ResourceResolver resourceResolver, QueryBuilder queryBuilder) {
        try {
            ...
            log.error("JsonProcessingException in createJsonFeed : {} ",e.getMessage());
        } catch (RepositoryException e) {
			log.error("RepositoryException in createJsonFeed : {} ",e.getMessage());
		}
    }
    ...
    private void getResearcherSchemaData(ResearcherSchema rs,Node componentNode,PageManager pageManager) throws RepositoryException {
        ...
        rs.setUrl(pageManager.getContainingPage(componentNode.getPath()).getPath().concat(".html"));
        ...
        if(componentNode.hasProperty("source")) {
            ...
            rs.setSource(Collections.EMPTY_LIST);
        }
        ...
        if(componentNode.hasProperty("url")){
            rs.setRefURL(componentNode.getProperty("url").getValue().getString());
        } else {
            rs.setRefURL("null");
        }
        rs.setPdfURL("null");
    ...
    private Map<String, String> fetchPages(String parentPagePath) {
        ...
        queryMap.put("group.property", JcrResourceConstants.SLING_RESOURCE_TYPE_PROPERTY);
